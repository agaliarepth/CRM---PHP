...
<div id="main_content">
        ...
        <div id="contact_area">
            ...
            <h2 id="contact">COBRANZAS > EDITAR PAGO DEUDA ANTERIOR</h2>
            <div>
           <TABLE border="1">
           <tr style="background-color:#333; color:#FFF">
           <td colspan="7" align="center">DETALLE DEL  DEUDA</td>
           </tr>
          ...
          <td>NUMERO CUOTAS</td>
           ...
           <td><?php echo $res["dias_credito"]?></td>
           <td><?php echo $res["numcuotas"]?></td>
           <td><?php echo $res["saldo_inicial"]?></td>
           <td><?php echo $res["saldo_actual"]+$res3["monto"];?></td>
           ...
           <form action="" method="post"  class="notas" name="form" id="wizard">
            ...
            <table border="0">
            <tr>
            <td colspan="3"><label>Cliente</label><input  name="cliente" size="50" id="cliente" type="text" readonly/ value="<?php echo $res["nombre_cliente"]?>"></td>
           
            <td><label>Numero de Pago</label><input value="<?php echo $res["numcuotas"]?>" type="text" name="numpago"   readonly="readonly" size="3"id="numpago" />
               ...
               <td><label>Saldo Actual</label><input value="<?php echo $res["saldo_actual"]+$res3["monto"]?>" type="text" name="saldo_actual"   readonly="readonly" size="10"id="saldo_actual" />
               
               </td>
               <td><label>Moneda</label>
               <select name="moneda">
               <option <?php if($res3["moneda"]=="Bs"){?> selected="selected"<?php }?>value="Bs">Bolivianos</option>
                <option <?php if($res3["moneda"]=="Sus"){?> selected="selected"<?php }?>value="Sus">Dolares</option>
               ...
               <tr>
            <td><label>Monto Pago</label><input  type="text"  size="15"name="monto" id="monto" value="<?php  echo $res3["monto"]?>"/></td>
             <td><label>Fecha del Pago</label><input  type="text"  class="fechas" id="fecha" name="fecha" value="<?php  echo date("d-m-Y",strtotime($res3["fecha"])); ?>"/></td>
              <td><label>Num Factura</label><input  type="text" size="15" name="numfactura"  style="text-transform:uppercase"id="numfactura" value="<?php  echo $res3["numfactura"]?>"/></td>
              <td><label>Num Recibo</label><input  type="text" size="15" name="numrecibo"  style="text-transform:uppercase"id="numrecibo" value="<?php  echo $res3["numrecibo"]?>"/></td>
               <td><label>Tipo Pago</label><select name="tipopago" id="tipopago" >
               <option <?php if($res3["tipopago"]=="EFECTIVO"){?> selected="selected"<?php }?> value="EFECTIVO">EFECTIVO</option>
               <option <?php if($res3["tipopago"]=="DEPOSITO"){?> selected="selected"<?php }?> value="DEPOSITO">DEPOSITO</option>
               </select>
               </td>
               <td  colspan="2">
                     <label>BANCO-NUMERO DE CUENTA</label>
                     <input type="TEXT" name="cuentabanco"   value="<?php echo $res3["cuentabanco"]; ?>"size="50" id="cuentabanco" <?php if($res3["tipopago"]=="DEPOSITO"){?> style="text-transform:uppercase; display:block" <?php } else{?>style="text-transform:uppercase; display:none;";<?php }?> />
                     </td>
            </tr>
            </table>
            </fieldset>
            <fieldset><legend><input type="checkbox"  value="0"  id="descuento"/>DESCUENTO</legend>
                     <table  id="tabladescuento"  <?php if(isset($res2["iddescuentoPago"])){?>style="display:block;"<?php } else{ ?>style="display:none;"<?php }?>>
                     <TR>
                     
                    
                    <td>
                     <label>Monto del Descuento</label>
                     <input  type="text" id="montodescuento" name="montodescuento" value="<?php echo $res2["monto"];?>"/>
                     </td>
                     <td>
                     <label>Descripcion Del Descuento</label>
                     <input  type="text"  size="50"id="descripciondescuento" name="descripciondescuento" value="<?php echo $res2["descripcion"];?>"/>
                     </td>
                     </TR>
		...
		<td valign="top" colspan="3">
            ...
            <input type="hidden" name="editar" value="editar"/>
             <input type="hidden" name="tienedescuento" id="tienedescuento"  <?php if(count($res2)>0){?>value="0"  <?php } else{?> value="0" <?php }?>/>
             <input type="hidden" name="idclientes" id="idclientes"  value="<?php echo $res3["idcliente"]?>"/>
             <input type="hidden" name="iddeudas" id="iddeudas"  value="<?php echo $res3["deudas_iddeudas"]?>"/>
               <input type="hidden" name="idPago" id="idPago" value="<?php echo $res3["idpagoVentasCredito"];?>" />
                <input type="hidden" name="iddescuento" id="iddescuento" value="<?php echo $res2["iddescuentoPago"];?>" />
                               <input type="hidden" name="valorcambio" id="valorcambio" value="<?php echo $tc2["valor"];?>" />
                                <input type="hidden" name="monto_ant" id="monto_ant"   value="<?php echo $res3["monto"]?>"/>



			<input type="button" value="Cancelar" id="cancelar"  onclick="javascript:window.location='<?php config::ruta()?>?accion=addPagos&id=<?php echo $res["clientes_idclientes"]?>';" />
		</td>
		<td></td>
	</tr>
